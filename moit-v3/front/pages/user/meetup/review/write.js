  ...
  useEffect(() => {
    ...
    const checkMeetupStatus = async () => {
      try {
        ...
        const response = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/meetups/${targetMeetupId}`, {
          headers: {
    ...
    const handleSuccessActions = async () => {
      if (success) {
        ...
        if (queryNotificationId) {
          try {
            await axios.patch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/notifications/reviews/${queryNotificationId}/read`);
          } catch (err) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (newFiles.length > 0) {
        ...
        const imageResponse = await axios.post(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/reviews/images`, formData, {
          headers: {
