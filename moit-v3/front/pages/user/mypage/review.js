...
function UserMyReviewPage() {
  ...
  const fetchNotifications = async () => {
    if (!memberId) return;
    try {
      const response = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/notifications/reviews?memberId=${memberId}`);
  ...
  const handleNotificationClick = async (notificationId, meetupId) => {
    try {
      await axios.patch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/notifications/reviews/${notificationId}/read`);
    } catch (error) {
