...
public class MemberServiceImpl implements MemberService{
	...
	public UserDto signup(UserDto dto) {
		...
		if (leakCount > 0) {
			throw new IllegalArgumentException("사용하려는 비밀번호가 과거 데이터 유출에 포함된 적이 있습니다. 다른 비밀번호를 사용해주세요.");
		}

		if (leakCount == -1) {
			throw new IllegalArgumentException("비밀번호 보안 검증에 실패했습니다. 잠시 후 다시 시도해주세요.");
		}
