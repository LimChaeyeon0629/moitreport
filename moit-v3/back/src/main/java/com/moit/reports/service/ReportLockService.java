...
public class ReportLockService {
    ...
    public boolean tryLock(Long reportId) {
        String lockKey = LOCK_PREFIX + reportId;				// "lock:report:reportId"

        Boolean acquired = redisTemplate.opsForValue()			// "lock:report:reportId"이라는 이름의 값이 없으면
                .setIfAbsent(									// "LOCKED" 저장 & 10초 후 자동 삭제
                        lockKey,
                        "LOCKED",
