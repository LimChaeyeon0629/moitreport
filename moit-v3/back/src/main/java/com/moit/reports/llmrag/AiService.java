...
public class AiService { // PDFBox / OpenAI API 호출
	...
	public String askToGptWithContext(String context, AiReportAnalysisDto reportContext) {
		...
		String systemInstruction = """
				...
				- 신고 대상 원문을 확인했을 때 신고자가 주장한 정책 위반 요소가 존재하지 않고,
				  원문에서 확인되는 내용이 일반적인 모임 안내 또는 운영 기준상 허용되는 범위에 해당하는 경우
				- 단순히 자료가 부족해서 위반 여부를 확인할 수 없는 경우는 B가 아니라 C로 판단
				...
				- APPROVED = 신고 승인
				  → 신고자의 신고 내용이 타당하고,
				    신고 대상에서 운영 정책 위반이 확인된 경우
				
				- REJECTED = 신고 반려
				  → 신고 대상에서 운영 정책 위반이 확인되지 않거나,
				    신고자의 주장을 뒷받침하는 위반 근거가 없는 경우
				
				주의:
				APPROVED는 "신고 대상 게시글을 승인한다"는 의미가 아닙니다.
				REJECTED는 "신고 대상 게시글을 거부한다"는 의미가 아닙니다.
				...
				- 없으면 '직접적으로 유사한 사례 없음'
  			    ...
  			    4. 판단 근거
				- 신고 승인(APPROVED) 근거와 신고 반려(REJECTED) 근거를 구분하여 작성
				- 형식:
				  신고 승인(APPROVED) 근거: ...
				  신고 반려(REJECTED) 근거: ...
				
				- 신고 승인(APPROVED) 근거에는 신고 대상의 운영 정책 위반을 뒷받침하는 확인된 사실만 작성하세요.
				- 신고 반려(REJECTED) 근거에는 신고 대상이 운영 정책을 위반하지 않았음을 뒷받침하는 확인된 사실만 작성하세요.
				- 단순한 자료 부족, 증거 부족, 확인 불가만을 신고 반려(REJECTED) 근거로 사용하지 마세요.
				
				- 단, 신고 대상 원문이 충분히 제공되어 있고,
				  신고자가 주장하는 위반 요소가 원문에서 확인되지 않으며,
				  원문에서 확인되는 내용이 운영 기준상 위반에 해당하지 않는 경우에는
				  이를 비위반 근거(B) 및 신고 반려(REJECTED) 근거로 사용할 수 있습니다.
