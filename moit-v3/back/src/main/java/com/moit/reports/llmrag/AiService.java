...
public class AiService { // PDFBox / OpenAI API 호출
	...
	public String askToGptWithContext(String context, AiReportAnalysisDto reportContext) {
		...
		String systemInstruction = """
				...
				3. 관련 운영 기준
				- report-policy.pdf에서 현재 쟁점에 직접 적용되는 기준만 1~2문장으로 요약
				...
				- APPROVED: XX%
				- REJECTED: XX%
				- 추천 검토 방향: 승인 우세 / 반려 우세 / 관리자 추가 검토 필요 중 하나
				- 판단 이유: 위의 사실, 운영 기준, 유사 사례 및 판단 근거만 사용하여 2~3문장으로 작성
				- 새로운 사실이나 근거를 추가하지 말 것
				- 자료 부족만을 이유로 승인 또는 반려를 선택하지 말 것
				
				7. 추가 확인 및 참고
				- 추가 확인 필요 사항은 MOIT의 신고 처리 판단에 직접 영향을 주는 사실만 작성
				- 법률 위반 여부, 형사 처벌 가능성, 의료적 위험 등
				  제공 문서의 범위를 벗어난 외부 전문 판단을 요구하지 마세요.
				- 게시글 자체의 문구만으로 운영 정책 위반 여부를 판단할 수 있다면
				  실제 행동 발생 여부를 불필요하게 추가 확인 사항으로 요구하지 마세요.
				- 없으면 '없음'
				
				- 참고 문서: 실제 분석에 사용한 문서명만 작성
				""";
